package com.team6560.lib.subsystems.roller;
...
import static com.team6560.lib.util.NetworkTable.NtValueDisplay.ntDispTab;
    ...
    public void build() {
        for (int i = 0; i < this.motors.size(); i++) {
            ...
            if (motorMode == MotorMode.VELOCITY) {
                ntDispTab(name).add("Motor "+(i+1)+" Velocity: ", motor.getMotor()::getVelocityRPM);
            } else {
                ntDispTab(name).add("Motor "+(i+1)+" Duty Cycle %: ", motor.getMotor()::getDutyCyclePercent);
            }
        }
